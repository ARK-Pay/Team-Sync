...
const FileExplorer = ({ 
  ...
  const fetchFiles = async (path = '') => {
    try {
      ...
      const fetchPromise = FileService.listFiles(path);
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      await FileService.uploadFile(files[0], currentPath);
  ...
  const handleFolderUpload = async (event) => {
    ...
    try {
      ...
      await FileService.uploadMultipleFiles(files, currentPath);
  ...
  const renderTreeItem = (item, depth = 0) => {
    ...
    return (
      <div key={item.path} className="file-explorer-item">
        <div 
          className={`flex items-center py-1 px-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded cursor-pointer ${isSelected ? 'bg-blue-100 dark:bg-blue-900' : ''}`}
  ...
  const buildFileTree = (files) => {
    ...
    sortedFiles.forEach(file => {
      ...
      if (fileMap[parentPath]) {
        ...
        // This handles cases where the backend returns files without their parent folders
        let currentPath = '';
